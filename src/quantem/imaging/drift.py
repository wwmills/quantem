...
class DriftCorrection(AutoSerialize):
    ...
    def align_translation(
        ...
        **kwargs,
        ...
        #         mode="constant",
        #         constant_values=np.median(cropped),
        #     )
        #     self.images_warped.array[a0] = padded
        #     self.interpolator[a0].set_pad_value(np.median(cropped))
        ...
        if return_images_shift:
            images_shift = np.zeros(
                (len(self.images), self.images[0].shape[0], self.images[0].shape[1])
            )
            # dxy are our shifts
            print(dxy)
            for ind in range(self.shape[0]):  # over all, not 1:
                images_shift[ind] = np.roll(
                    np.pad(
                        self.images[ind].array,
                        ((max_image_shift, max_image_shift), (max_image_shift, max_image_shift)),
                        mode="median",
                    ),
                    dxy[ind],
                    axis=(0, 1),
    ...
    def generate_corrected_image(
        ...
        **kwargs : dict
        ...
        if mask_output or return_validity_mask:
        ...
        if output_original_shape:
            ...
            mask = np.real(
                np.fft.ifft2(
                    fourier_cropping(np.fft.fft2(mask), self.shape[-2:]) / upsample_factor**2
                )
            )
