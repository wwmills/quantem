...
class geometric_phase_analysis_2D(AutoSerialize):
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        self.mask_size = None
        self.defined_peaks = np.zeros(self.shape[0], dtype = bool) # set to true when the peaks have been set
        if show_images:
    ...
    def calculate_phase_map(
        ...
        if shift_back:
            shift_phase_back = np.exp(1j*2*np.pi*(-shift[0]*xx-shift[1]*yy))
            G_matrix = ifft2(ifftshift(mask*fftshift(fft2(G_matrix*shift_phase_back))))    # Without hamming
